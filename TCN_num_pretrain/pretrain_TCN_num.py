...
sys.path.append("../../")
from Methods.TCN_num_pretrain.model_pretrain import TimeSlicePre
from Methods.utils import data_generator_num,data_get_retweets
...
#retweets_test = data_get_retweets(filename=args.file_dir+args.filename_test)
#X_train, Y_train = data_generator_num(filename=args.file_dir+args.filename_train,ob_time=ob_time, gap_time=args.interval_time)
#X_val,Y_val = data_generator_num(filename=args.file_dir+args.filename_val,ob_time=ob_time, gap_time=args.interval_time) 
#X_test, Y_test = data_generator_num(filename=args.file_dir+args.filename_test,ob_time=ob_time, gap_time=args.interval_time)
...
def get_batch(i,batch_size,retweets,args,if_print = False):
    ...
    for idx_batch in range(len(batch_retweets)):
        ...
        #print("retweets, last time slice, label range:",temp_retweets,last_time_slice,label_range)
        #print("last time slice, label range:",last_time_slice,label_range)
...
for epoch in range(1, epochs + 1):
    ...
    for i in range(0, len(retweets_train), batch_size):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            val_loss_list = []
            # random sample 100 batch in validation data set 
            for jj in range(0,100):
            #for j in range(0, len(retweets_val), batch_size):
                #j = random.randint(0,len(retweets_val)-1)
                j = random.randrange(0,len(retweets_val),batch_size)
                if jj == 0:
                    x1_val, x2_val, y_val = get_batch(j, batch_size, retweets_val,args,True)
                else:
                ...
                if jj == 0:
                    print("true:",torch.flatten(y_val))
                    print("pred:",torch.flatten(predy_val))
                    print("y_concat:",model.get_concat())
            val_loss = np.mean(val_loss_list)

            if val_loss < Best_val_loss:
                Patience = max_try
                Best_val_loss = val_loss
                torch.save(model,'./save_models/pretrain_twitter_model_max_label'+str(args.max_label)+"_time_slices"+str(args.time_slices)+"_lr"+str(args.lr)+"_wd"+str(args.weight_decay)+'.pkl')
